# Activa StellarDaybook en el desktop (Nexus): venv, config, arranque ahora + Inicio de Windows.
...
$log = Join-Path $repo "_nexus_activation_log.txt"
...
"=== StellarDaybook Nexus $(Get-Date -Format o) ===" | Out-File -FilePath $log -Encoding utf8
Write-Log "Repo: $repo"
...
if (Test-Path -LiteralPath $cfg) {
    ...
    if ($raw -match '(?m)^(\s*name:\s*).+$') {
        ...
        Write-Log "config.local.yaml listo (name ya Nexus)."
    } else {
        Write-Log "config.local.yaml listo (name ya otro valor)."
    }
}
